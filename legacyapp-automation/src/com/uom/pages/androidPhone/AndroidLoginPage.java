...
public class AndroidLoginPage extends LoginPage {
	...
	public static final String[] rbtn_SaveUserNameChecked = {"//android.widget.CheckBox[@resource-id='save-username' and @checked='{dynamic1}']",XPATH,"rbtn_SaveUserNameChecked"};
	...
	public String[] findChildObject(String reqLocatorName){	
		Field childObjects;
		try {
			childObjects = AndroidLoginPage.class.getDeclaredField(reqLocatorName);
		
		String[] childObject;
		String[] objForChild={"","",""};
		if(childObjects.getType().isInstance(objForChild)){
			childObject=(String[]) (AndroidLoginPage.class.getDeclaredField(reqLocatorName).get(null));
			return childObject;
		}
